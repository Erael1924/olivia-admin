...
if (props.result.data.has_variant == 1) {
    if (props.variants && props.variants[0]) {
        let arr = props.result.data.variants;
        let options = [];
        let secondOptions = [];
        const seenNames = new Set();
        props.options.forEach((item) => {
            if (!seenNames.has(item.first_option)) {
                seenNames.add(item.first_option);

                options.push({
                    id: item.id,
                    name: item.first_option,
            ...
            if (!seenNames.has(item.second_option)) {
                seenNames.add(item.second_option);

                secondOptions.push({
                    id: item.id,
                    name: item.second_option,
                    images: [],
                });
            }

            if (
                !priceStock.value[item.first_option + "-" + item.second_option]
            ) {
                priceStock.value[item.first_option + "-" + item.second_option] =
        ...
        variants.value.push({
            id: props.variants[0].id,
            name: props.variants[0].name,
            options: options,
        });

        variants.value.push({
            id: props.variants[1].id,
            name: props.variants[1].name,
...
const form = useForm({
    form: 1,
    thumbnail: props.result.data.thumbnail ?? null,
    collection: props.result.data.collection.id ?? null,
    category: props.result.data.category.id ?? null,
    sub_category: props.result.data.sub_category.id ?? null,
    tags: props.result.data.tags
        ? props.result.data.tags
              .split(",")
              .map((tag) => parseInt(tag.trim(), 10))
        : [],
    product_code: null,
    product_name: props.result.data.name ?? null,
    description: props.result.data.description ?? null,
    has_variant: props.result.data.has_variant ?? null,
    price: props.result.data.price ?? null,
    stock: props.result.data.stock ?? null,
    product_images: props.result.data.images ?? [],
    removed_product_images: [],
    product_details: props.result.data.details ?? [],
    weight: props.result.data.weight ?? null,
    width: props.result.data.width ?? null,
    length: props.result.data.length ?? null,
    height: props.result.data.height ?? null,
    variants: [],
    removed_variants: [],
    removed_variant_options: [],
    variants_info: [],
});

thumbnail.value = form.thumbnail;
...
const create = () => {
    ...
    form.put(route("products.update", { id: props.result.data.id }), {
        preserveScroll: true,
        onError: () => {
            toastStore.addToast({
                type: "error",
...
const removeProductImage = (imageIndex) => {
    if (form.product_images[imageIndex]) {
        if (form.product_images[imageIndex].hasOwnProperty("id")) {
            form.removed_product_images.push(
                form.product_images[imageIndex].id
            );
        }
...
const removeVariantOption = (index, optionIndex) => {
    if (index == 0) {
        for (let x = 0; x < variantsInfo.value[optionIndex].length; x++) {
            if (variantsInfo.value[optionIndex][x].hasOwnProperty("id")) {
                form.removed_variant_options.push({
                    id: variantsInfo.value[optionIndex]["id"],
                    type: "primary_variant_option",
                });
            }
        }
        variantsInfo.value.splice(optionIndex, 1);
    } else {
        for (let x = 0; x < variants.value[0].options.length; x++) {
            if (variantsInfo.value[x][optionIndex].hasOwnProperty("id")) {
                form.removed_variant_options.push({
                    id: variantsInfo.value[x][optionIndex]["id"],
                    type: "secondary_variant_option",
                });
            }

            variantsInfo.value[x].splice(optionIndex, 1);
        }
    }
...
const removeVariant = (index) => {
    if (variants.value[index].hasOwnProperty("id")) {
        form.removed_variants.push(variants.value[index]["id"]);
    }
    for (let x = 0; x < variants.value[0].options.length; x++) {
        for (let y = 0; y < variants.value[1].options.length; y++) {
            console.log(variants.value[1].options);
            console.log(variantsInfo.value);
            if (variantsInfo.value[x][y].hasOwnProperty("id")) {
                form.removed_variant_options.push({
                    id: variantsInfo.value[x][y]["id"],
                    type: "secondary_variant_option",
                });
            }
        }
        for (let y = 0; y < variants.value[1].options.length; y++) {
            variantsInfo.value[x].splice(y, 1);
        }
    }
...
const setupVariation = () => {
    let firstVariant = variants.value[0].options;
    let secondVariant = variants.value[1].options;
    variantsInfo.value = [];

    for (let x = 0; x < firstVariant.length; x++) {
        let arr = [];
        for (let y = 0; y < secondVariant.length; y++) {
            let key = firstVariant[x].name + "-" + secondVariant[y].name;
            if (priceStock.value.hasOwnProperty(key)) {
                arr.push({
                    id: priceStock.value[key].id,
                    price: priceStock.value[key].price,
                    stock: priceStock.value[key].stock,
                });
            } else {
